...
def solve(window_size=10, number_of_clusters=5, lambda_parameter=11e-2, beta=400, maxIters=1000, threshold=2e-5,
          write_out_file=False, input_file=None, prefix_string="", num_proc=1):
    '''
    ...
    for iters in xrange(maxIters):
        ...
        if iters != 0:
            cluster_norms = [(np.linalg.norm(old_computed_covariance[num_clusters, i]), i) for i in
                             xrange(num_clusters)]
            ...
            for cluster in xrange(num_clusters):
                if len_train_clusters[cluster] == 0:
                    ...
                    start_point = np.random.choice(
                        train_clusters[cluster_selected])  # random point number from that cluster
                    for i in range(0, cluster_reassignment):
                        ...
                        cluster_mean_info[num_clusters, cluster] = complete_D_train[point_to_move, :][
